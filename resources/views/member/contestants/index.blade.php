        ...
        <div class="max-w-7xl mx-auto">
            <div class="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center">
                <div>
                    <a href="{{ route('member.dashboard') }}"
                        class="text-indigo-600 hover:text-indigo-800 flex items-center mb-2 font-medium">
                    ...
                    <p class="mt-2 text-slate-600">Cast your vote carefully. You only get <span
                            class="font-bold text-indigo-600">ONE vote</span> for this round.</p>
                    <div
                        class="mt-2 inline-flex items-center px-3 py-1 rounded-full bg-slate-200 text-slate-700 text-xs font-bold uppercase tracking-wider">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                @forelse($voting->votingContestants as $votingContestant)
                    ...
                    <div
                        class="bg-white rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 border border-slate-100 flex flex-col {{ $votedContestantId == $contestant->id ? 'ring-4 ring-green-400' : '' }}">
                        <div class="relative h-80 overflow-hidden bg-gray-200 group">
                            ...
                            @endif
                            <div
                                class="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-90">
                            </div>
                        ...
                        <div class="p-5 flex-grow flex flex-col justify-between">
                            <div>
                                ...
                                <div class="mt-4 border-t border-slate-100 pt-4">
                                    <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Q&A with
                                        Members</h4>
                                    ...
                                    <button onclick="toggleQuestionForm({{ $contestant->id }})"
                                        class="mt-3 text-xs font-bold text-indigo-600 hover:text-indigo-800 transition-colors">
                                        + Ask her a question
                                    </button>

                                    <div id="q-form-{{ $contestant->id }}" class="hidden mt-3">
                                        <form action="{{ route('member.contestant.question.store', $contestant) }}"
                                            method="POST">
                                            @csrf
                                            ...
                                            <div class="mt-2 flex justify-end gap-2">
                                                <button type="button"
                                                    onclick="toggleQuestionForm({{ $contestant->id }})"
                                                    class="text-xs text-slate-400 hover:text-slate-600">Cancel</button>
                                                <button type="submit"
                                                    class="text-xs bg-indigo-600 text-white px-3 py-1 rounded-md hover:bg-indigo-700 transition-colors">Submit</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                            ...
                            <form
                                action="{{ route('member.vote.store', ['voting' => $voting->voting_id, 'contestant' => $contestant->id]) }}"
                                ...
                                {{ $votedContestantId == $contestant->id
                                    ...
                                    : ($votedContestantId
                                        ...
                                        : 'bg-gradient-to-r from-red-500 to-pink-600 text-white hover:from-red-600 hover:to-pink-700 hover:shadow-red-500/30') }}"
