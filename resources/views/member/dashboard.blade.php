    ...
    <div class="min-h-screen bg-slate-50">
        ...
        <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="mb-8">
                ...
                <!-- <p class="text-slate-500 mt-1">Welcome back, {{ Auth::user()->name }}! Manage your votes and view results.
                </p>
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <section>
                        ...
                        {{-- <div class="flex items-center justify-between mb-4">
                            ...
                            <span class="px-2 py-1 bg-green-100 text-green-700 text-xs font-bold rounded uppercase">Global
                                Sync</span>
                        ...
                        {{-- @if (isset($activeRounds) && $activeRounds->count() > 0)
                            ...
                            <div class="grid grid-cols-1 gap-4">
                                @foreach ($activeRounds as $round)
                                    <div
                                        class="bg-white border border-slate-200 rounded-xl p-6 hover:shadow-sm transition-shadow">
                                        <div
                                            class="flex flex-col md:flex-row justify-between md:items-center space-y-4 md:space-y-0">
                                            <div>
                                                <div class="flex items-center space-x-2 mb-1">
                                                    <span
                                                        class="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded uppercase tracking-wider">
                                                        {{ $round->type ?? 'Regional' }} Round
                                                    </span>
                                                    <span class="text-xs text-slate-400">•</span>
                                                    <span class="text-xs text-slate-500 font-medium">Ends in: 12h 45m
                                                        30s</span>
                                                </div>
                                                <h3 class="text-xl font-bold text-slate-900">{{ $round->title }}</h3>
                                            </div>
                                            <div class="flex space-x-2">
                                                <a href="{{ route('member.results.show', $round->voting_id) }}"
                                                    class="px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50 border border-slate-200 rounded-lg transition">
                                                    Leaderboard
                                                </a>
                                                <a href="{{ route('member.voting.show', $round->voting_id) }}"
                                                    class="px-4 py-2 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow-sm transition">
                                                    Vote Now
                                                </a>
                                            </div>
                                        </div>
                                        <div
                                            class="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between text-xs text-slate-500">
                                            <span class="flex items-center"><svg class="w-3 h-3 mr-1" fill="currentColor"
                                                    viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd"
                                                        d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z"
                                                        clip-rule="evenodd"></path>
                                                </svg> Global Start: {{ $round->creationdate }}</span>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                        ...
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <span
                                class="px-2 py-0.5 bg-indigo-50 text-indigo-700 text-[10px] font-bold rounded uppercase">Active</span>
                        </div>
                        ...
                        <div class="flex flex-col space-y-2">
                            ...
                            <div class="flex justify-between text-xs">
                                ...
                                <span
                                    class="text-slate-900 font-bold">{{ isset($member) && $member->subscription_ends_at ? \Carbon\Carbon::parse($member->subscription_ends_at)->format('M d, Y') : 'N/A' }}</span>
                            </div>
